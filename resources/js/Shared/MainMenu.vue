<template>
  <div>
    ...
    <div class="mb-4">
      <inertia-link class="flex items-center group py-3" :href="route('testing')">
        <icon name="printer" class="w-4 h-4 mr-2" :class="isUrl('testing') ? 'fill-white' : 'fill-indigo-400 group-hover:fill-white'" />
        <div :class="isUrl('testing') ? 'text-white' : 'text-indigo-300 group-hover:text-white'">Testing</div>
      </inertia-link>
    </div>
  </div>
</template>
